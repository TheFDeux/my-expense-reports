...
- **Six-month trend bars are stacked by type** (requested) in the same colours as the donut, with each month's total printed above its bar (compact form like "1,2 k€" when space is short) and a legend below; tapping a bar lists its breakdown.
